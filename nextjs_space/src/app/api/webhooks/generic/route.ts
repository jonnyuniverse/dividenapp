...
import { processGenericEvent, MappingRule } from '@/lib/webhook-actions';
...
export async function POST(req: NextRequest) {
  ...
  const auth = await authenticateFromBody(req, 'generic', bodyText);
  ...
  const results = await processGenericEvent(payload, auth.userId, mappingRules, fieldMap);
  const hasErrors = results.some(r => !r.success);

  if (!config && !mappingRules) {
    learnAndSaveMapping(auth.webhookId, payload, 'generic').catch(() => {});
  }
