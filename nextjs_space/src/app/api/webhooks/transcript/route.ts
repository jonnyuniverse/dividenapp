...
import { processTranscriptEvent, MappingRule } from '@/lib/webhook-actions';
...
export async function POST(req: NextRequest) {
  ...
  const auth = await authenticateFromBody(req, 'transcript', bodyText);
  ...
  const results = await processTranscriptEvent(payload, auth.userId, mappingRules, fieldMap);
  const hasErrors = results.some(r => !r.success);

  if (!config && !mappingRules) {
    learnAndSaveMapping(auth.webhookId, payload, 'transcript').catch(() => {});
  }
